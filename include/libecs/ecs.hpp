#ifndef LIBECS_ECS_HPP
#define LIBECS_ECS_HPP
...
class ECS {
   public:
    ...
    template <typename T>
    std::vector<std::pair<Entity, T>> &get_all() {
        assert(registered<T>());
        return static_cast<Store<T> &>(*stores_[T::id]).data();
    }
    ...
    void print() const {
        ...
        std::cout << "Avg. components: " << std::setprecision(2) << (float)total / entities_.size()
                  << std::endl;
    }
