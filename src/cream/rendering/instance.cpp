...
namespace cream
{
    ...
    void Instance::buffer_mesh(const Mesh &mesh)
    {
        ...
        // Colours
        glBindBuffer(GL_ARRAY_BUFFER, cbo_);
        glBufferData(GL_ARRAY_BUFFER, mesh.colours().size()*sizeof(glm::vec3), &mesh.colours()[0], GL_STATIC_DRAW);
        // Normals
        glBindBuffer(GL_ARRAY_BUFFER, nbo_);
        glBufferData(GL_ARRAY_BUFFER, mesh.normals().size()*sizeof(glm::vec3), &mesh.normals()[0], GL_STATIC_DRAW);
    }

    void Instance::buffer_instances(
    ...
    )
    {
        ...
        // Rotations
        glBindBuffer(GL_ARRAY_BUFFER, rbo_);
        glBufferData(GL_ARRAY_BUFFER, rotations.size()*sizeof(glm::vec3), &rotations[0], GL_STATIC_DRAW);
        // Translations
        glBindBuffer(GL_ARRAY_BUFFER, tbo_);
